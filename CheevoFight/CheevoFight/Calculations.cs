...
internal class Calculations
{
    ...
    public static Dictionary<string, HashSet<string>> TagsByGameName { get; set; }
    private static Dictionary<string, string> GameNameByCapsuleImagePathDisk { get; set; }
    public static Dictionary<string, Dictionary<string, HashSet<string>>> MutuallyOwnedGameNamesByTagByFriendSteamId { get; set; }
    ...
    static Calculations()
    {
        ...
        Calculations.TagsByGameName = new Dictionary<string, HashSet<string>>();
        Calculations.GameNameByCapsuleImagePathDisk = new Dictionary<string, string>();
    ...
    private static async Task<string> GetSteamIdFromProfileURLAsync()
    {
        ...
        {
            TextBlockContentTask = "Retrieving Steam Id",
            ProgressBarValue = 0,
        ...
        if (ViewModel.SteamProfileURL.Left(standardURLPrefix.Length).Equals(standardURLPrefix))
        {
            var tentativeSteamId = ViewModel.SteamProfileURL.Substring(ViewModel.SteamProfileURL.LastIndexOf("/7") + 1);
            if (tentativeSteamId.Right(1).Equals('/'))
            {
                tentativeSteamId = tentativeSteamId.Left(tentativeSteamId.Length - 1);
            }

            return tentativeSteamId;
        }
    ...
    private static async Task SetPlayerSteamNameAndAvatarAsync(string steamId)
    {
        ...
        ProgressBarManager.SetTextBlockContentTask("Default", "Retrieving your Steam name and avatar");
    ...
    private static async Task SetPlayerOwnedAppIdsAsync(string steamId)
    {
        ProgressBarManager.IncrementValue("Default");
        ProgressBarManager.IncrementMaximum("Default", 1);
        ProgressBarManager.SetTextBlockContentTask("Default", "Retrieving your owned games");
    ...
    {
        ProgressBarManager.SetTextBlockContentTask("Default", "Retrieving your friends' Steam names and avatars");
    ...
    {
        ProgressBarManager.SetTextBlockContentTask("Default", "Retrieving your mutually owned games with each friend");
    ...
    private static async Task SetPlayerAchievementsByAppIdAsync(string steamId)
    {
        ProgressBarManager.IncrementValue("Default");
        ProgressBarManager.IncrementMaximum("Default", 1);
        ProgressBarManager.SetTextBlockContentTask("Default", "Retrieving your achievement count per game");
    ...
    )
    {
        ...
        for (var i = 0; i < numberOfRunsNeeded; i++)
        {
            ...
            foreach (var appIdIndicator in allAppIdIndicators)
            {
                ...
                {
                    var unlockedAchievementsBlock = positionOfNextAppId == -1
                        ? jSONAchievements.Substring(
    ...
    private static int GetEndPositionOfAppIdInTopAchievementsEntry(string? jSONAchievements, int startPosition)
    {
        ...
        // No Achievements, end of file, "}", "," == -1
        if (endPositionComma == -1)
        {
            return endPositionRightBrace;
        }

        // Achievements, middle of file, ",", "}" != -1
        // No achievements, middle of file, "}", "," != -1
        // Achievements, end of file, ",", "}" != -1
    ...
    private static async Task SetAchievementsByAppIdByFriendSteamIdAsync()
    {
        ...
        ProgressBarManager.SetTextBlockContentTask("Default", "Retrieving friends' achievement counts per mutually owned game");
    ...
    private static async Task SetCapsuleAndTagsByAppIdAsync()
    {
        ...
        ProgressBarManager.SetTextBlockContentTask("Default", "Retrieving game tags from store pages");
        ...
        foreach (var appId in Calculations.PlayerGamesOwnedNameByAppId.Keys)
        {
            ...
            var pathPotentialCachedTags = Path.Join(
                Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "CheevoFight cache", appId + ".txt"
            );
            if (Calculations.CapsuleAndTagsByAppId.ContainsKey(appId) && !File.Exists(pathPotentialCachedTags))
            {
                await Calculations.UpdateTagsWithHTMLofSteamStorePageAsync(appId);

                if (ViewModel.CacheCapsulesAndTags)
                {
                    File.WriteAllLines(pathPotentialCachedTags, Calculations.CapsuleAndTagsByAppId[appId].Item2);
                }
            }
    ...
    {
        ProgressBarManager.IncrementMaximum("Default", Calculations.PlayerGamesOwnedNameByAppId.Count);
        ProgressBarManager.SetTextBlockContentTask("Default", "Retrieving game genres and capsule images from store API");
        ...
        foreach (var appId in Calculations.PlayerGamesOwnedNameByAppId.Keys)
        {
            ...
            var pathPotentialCachedCapsuleImage = Path.Join(
                Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "CheevoFight cache", appId + ".jpg"
            );
            var pathPotentialCachedTags = Path.Join(
                Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "CheevoFight cache", appId + ".txt"
            );
            if (
                ViewModel.CacheCapsulesAndTags && 
                File.Exists(pathPotentialCachedCapsuleImage) && 
                File.Exists(pathPotentialCachedTags)
            )
            {
                var tags = File.ReadAllLines(pathPotentialCachedTags).ToHashSet();
                Calculations.CapsuleAndTagsByAppId.Add(appId, (pathPotentialCachedCapsuleImage, tags));
                Calculations.TagsByGameName.Add(Calculations.PlayerGamesOwnedNameByAppId[appId], tags);
                Calculations.GameNameByCapsuleImagePathDisk.Add(pathPotentialCachedCapsuleImage, Calculations.PlayerGamesOwnedNameByAppId[appId]);
            ...
            var capsule = await GetCapsuleImagePathFromStoreAPIJSONAsync(jSONAppDetails, appId);
            ...
            Calculations.TagsByGameName.Add(Calculations.PlayerGamesOwnedNameByAppId[appId], genres);
            Calculations.GameNameByCapsuleImagePathDisk.Add(capsule, Calculations.PlayerGamesOwnedNameByAppId[appId]);
            ProgressBarManager.IncrementValue("Default");
        }
    }
            ...
            catch (HttpRequestException exception)
            {
                switch (exception.Message)
                {
                    ...
                    case "Response status code does not indicate success: 429 (Too Many Requests).":
                        // Rate limit exceeded
                        ProgressBarManager.SetTextBlockContentTask("Default", "Waiting for more Steam store API requests to become available");
    ...
    private static async Task<string> GetCapsuleImagePathFromStoreAPIJSONAsync(string jSONAppDetails, string appId)
    {
        ...
        var pathDisk = ViewModel.CacheCapsulesAndTags
            ? Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "CheevoFight cache", appId + ".jpg")
            : Path.GetTempFileName();
        Directory.CreateDirectory(Path.GetDirectoryName(pathDisk));
        using var fileStream = File.Create(pathDisk);
    ...
    private static async Task UpdateTagsWithHTMLofSteamStorePageAsync(string appId)
    {
        ...
        Calculations.TagsByGameName.Remove(Calculations.PlayerGamesOwnedNameByAppId[appId]);
        Calculations.TagsByGameName.Add(Calculations.PlayerGamesOwnedNameByAppId[appId], tags);
    }
    ...
    {
        ProgressBarManager.IncrementMaximum("Default", Calculations.SteamNamesAndAvatarsByFriendSteamId.Count);
        ProgressBarManager.SetTextBlockContentTask("Default", "Calculating player % of achievements by friend by tag");
    ...
    private static void PrepareToDisplayResults(ViewModel viewModel)
    {
        ...
        foreach (var keyValuePair1 in Calculations.PlayerPercentageByMututallyOwnedTagByFriendSteamId)
        {
            ...
            foreach (var keyValuePair2 in playerPercentageByMututallyOwnedTag)
            {
                ...
                _ = new ResultsManager(
                    friendSteamId, tag, playerPercentage,
                    Calculations.MutuallyOwnedCapsuleImagesByTagByFriendSteamId[friendSteamId][tag],
                    Calculations.GameNameByCapsuleImagePathDisk
                );
            }
        ...
        viewModel.SearchResults = ViewModel.TagResultsBySteamName[firstFriendSteamName];
    }
